...
public class Configuration {
    ...
    /**
     * 由于ES的setting不支持数组josn对象，所以，采用自定义格式配置字典文件 dicName:词典名称 dicPath:词典路径，如果是远程，则为远程路径 isRemote:true:是远程字典文件，false:是本地文件
     * 格式说明：#为字段名称开始符，$是值开始符
     * @param absolutePath
     * @param str 自定义格式配置字典文件：#dicName$extra#dicPath$extra_test.dic#isRemote$false
     * @return
     */
    private DicFile str2DicFile(String absolutePath, String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (tmpChar == '#') {
                ...
                if (isName) {
                    fieldName[tmpCursor++] = tmpChar;
                } else {
                    fieldValue[tmpCursor++] = tmpChar;
                }
                if (str.length() == i + 1) {
                    setDicFile(fieldName, fieldValue, dicFile);
                }
