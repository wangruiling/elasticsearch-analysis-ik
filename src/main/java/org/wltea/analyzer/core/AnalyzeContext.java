...
public class AnalyzeContext {
    ...
    // 是否大小写过滤
    private boolean isEnableLowerCase;
    private boolean isUseSmart;
    private List<String> mainDicNames;
    private List<String> quantifierNames;
    private List<String> stopwordDicFile;

    public AnalyzeContext(Configuration configuration) {
        this.isUseSmart = configuration.isUseSmart();
        this.isEnableLowerCase = configuration.isEnableLowercase();
        ...
        configuration.getDicFiles().forEach(dicFile -> {
            if (dicFile.getDictType() == DicFile.DictType.INTACT_WORDS) {
                ...
                quantifierNames.add(dicFile.getDicName());
            } else if (dicFile.getDictType() == DicFile.DictType.STOPWORDS) {
                stopwordDicFile.add(dicFile.getDicName());
            }
        });
        mainDicNames = mainDicNames.stream().sorted((o1, o2) -> -1).collect(Collectors.toList());
        quantifierNames = quantifierNames.stream().sorted((o1, o2) -> -1).collect(Collectors.toList());
        stopwordDicFile = stopwordDicFile.stream().sorted((o1, o2) -> -1).collect(Collectors.toList());
    }
    ...
    public List<String> getMainDicNames() {
        return mainDicNames;
    }

    public List<String> getQuantifierNames() {
        return quantifierNames;
    }
    ...
    public void initCursor() {
        this.cursor = 0;
        this.segmentBuff[this.cursor] = CharacterUtil.regularize(this.segmentBuff[this.cursor], this.isEnableLowerCase);
    ...
    Lexeme getNextLexeme() {
        ...
        while (result != null) {
            ...
            if (Dictionary.getSingleton().isStopWord(this.stopwordDicFile, this.segmentBuff, result.getBegin(), result.getLength())) {
                //是停止词继续取列表的下一个
